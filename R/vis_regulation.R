...
if(reg_type == "nodes") {
      ...
      cut_df <- net_edges[[reg_by]] ## create vector with all values we should regulate by
...
## regulation if we only want to include a certain number of edges// Cut_off defines that number
    if(reg_type == "number"){
      cut_df <- sort(net_edges[,reg_by], decreasing = TRUE)
    ...
    if(reg_type == "value"){
      net_edges <- net_edges %>% dplyr::filter(internal_reg >= cut_off)
      net_edges <- net_edges %>% dplyr::select(-internal_reg)}
net_edges}
