...
width_max <- function(net_edges, edges_width_max){
      ...
      if(!is.numeric(edges_width_max) & edges_width_max != "auto") {stop("edges_width_max must be numeric or 'auto'")}
      if(edges_width_max == "auto"){edges_width_max <- as.numeric(max(net_edges$width))}
