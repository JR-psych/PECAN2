...
pecanVisEx <- function(data, p_id, edges, nodes, edges_sep,label_in_names = FALSE,labels_sep = "none",labels = "none", regulation_by = "none",cut_by = "none",cut_off = "none",nodes_color = "red",edges_color = "black",unweigthed,max10 = 10,arrows = "to",
  ...
  edges_ex <- edges
  nodes_ex <- nodes
  edges_sep_ex <- edges_sep
  label_in_names_ex <- label_in_names
  labels_sep_ex <- labels_sep
  labels_ex <- labels


  data_all <- pecanExtract(data = data_ex, p_id = p_id_ex, edges = edges_ex, nodes = nodes_ex, edges_sep = edges_sep_ex,
                           label_in_names = label_in_names_ex, labels_sep = labels_sep_ex,labels = labels_ex)

  net_edges <- data_all$edges
  net_nodes <- data_all$nodes
  cut_by <- cut_by
  cut_off <- cut_off
 ...
 # if(is.data.frame(edges) == FALSE){stop("edges must be a dataframe")}
  ...
  if("width" %in% colnames(net_edges) == FALSE){stop("'width' column in edges is missing")}
  if(any(net_edges[,"width"] < 0)){stop("no negative values in 'width' allowed")}
