...
#                      net_edges <- net_edges %>% mutate(shadow.enabled = ifelse(shadow_by <= sh_1,FALSE,TRUE),
...
#   net_edges <- net_edges %>% filter(net_edges[,cut_by] >= final_cutoff)}} ## filter our edges dataframe by our new cut_off
...
#   net_edges <- net_edges %>% filter(net_edges[,cut_by] >= final_cutoff)}}
## regulation if width variable should be higher than a certain value
#  if(regulation_by == "value"){if(is.numeric(cut_off)){cut = cut_off} else {stop("Haben wir noch Zahlen? 2 noch?")}
#       if(exists("cut_by") == FALSE){stop("OHneim! 404 cut_by not found")}
#      else{net_edges <- net_edges %>% filter(net_edges[,cut_by] >= cut_off)}}
...
# Width maximum auf 10 skalieren und den rest mit..
#if(exists("max10")){
# if(is.numeric(max10) & max10 > 0){
#  if(max10 < 10){net_edges <- net_edges %>% mutate(width = width*(10/max10))} else
#    {if(max10 > 10){net_edges <- net_edges %>% mutate(width = width/(max10/10))}}
#   } else {stop("max10 must be numeric")}}

#nodes <- nodes %>% mutate(color = "red")  not needed because red is default now


## When a ranking symstem should be used...
## smaller numbers equal higher ranks so 0 is the highest possible rank.

#Negative values are not allowed to this point
#if(use_ranking != "none"){if(any(is.numeric(net_edges[,use_ranking])) == FALSE){stop("Use_ranking must be numeric")}}
...
#if(use_ranking != "none"){
#  if(is.numeric(min_rank)){net_edges <- net_edges %>% filter(net_edges[,use_ranking] <= min_rank)} else{stop("min_rank is missing or is not numeric")}
#  }
#supi <<- regulation_by
...
#Setting nodes color. Bisschen komisch mit exits zu arbeiten, da default red ist. muss ich vllt noch ändern
#if(exists("nodes_color")){net_nodes <- net_nodes %>% mutate(color = nodes_color)}

# calculating range for edge range color setting



# assign("testi", net_edges, envir = .GlobalEnv)
