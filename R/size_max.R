...
size_max <- function(net_nodes, nodes_size_max){
  ...
  if(!is.numeric(nodes_size_max) & nodes_size_max != "auto") {stop("nodes_size_max must be numeric or 'auto'")}
  if(nodes_size_max == "auto"){nodes_size_max <- as.numeric(max(net_nodes$size))}
